...
int AntiConv::DetectFace(const cv::Mat & img_src, std::vector<FaceInfo>* faces)
{
	...
	ncnn::Mat in = ncnn::Mat::from_pixels_resize(img_cpy.data, ncnn::Mat::PIXEL_BGR2RGB, img_width, img_height, inputSize_.width, inputSize_.height);
	ex.input("data", in);
		...
		for (int h = 0; h < height; ++h)
		{
			for (int w = 0; w < width; ++w)
			{
				...
				for (int a = 0; a < anchor_num; ++a)
				{
					...
					// std::cout << "probabily: "<< prob << std::endl;
